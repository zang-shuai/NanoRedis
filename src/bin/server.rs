//! 这个文件是库中实现的服务器的入口点。它执行命令行解析并将参数传递给:
//! `nano_redis::server`.
//!
//! `clap`用于解析参数
...
use nano_redis::connect::server_start;

#[tokio::main]
pub async fn main() -> nano_redis::Result<()> {
    set_up_logging()?;
    ...
    server_start::run(listener, signal::ctrl_c()).await;
...
fn set_up_logging() -> nano_redis::Result<()> {
